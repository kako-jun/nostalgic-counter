...
`Nostalgic Counter（ノスタルジックカウンター）` は、インターネット黎明期のホームページでお馴染みだった「アクセスカウンター」を、いま風の技術で再現したものです。

- サイトの訪問者をカウントし、サイト内に埋め込み表示
- 数値に、アニメーション GIF も使える
- 連続カウント防止機能あり
- キリ番表示機能あり

などを実現します。

JavaScript が動作する環境ならば表示できるため、GitHub Pages などの静的サイトにも設置できます。

ソースコードを公開しているので、自分で借りたクラウドに設置することも、改造することも可能です。

## Description

### Demo

![demo](https://nostalgic-counter.llll-ll.com/demo)
...
操作は全て Web ブラウザで行います。
URL 欄 に、「したい操作を意味する文字列」を打ち込んで Enter を押すことで実行します。
...
誤って書いてしまうと、皆にパスワードがバレてしまいます。

#### 自分のカウンターに対して、設定を変更する

新規作成された直後のカウンターは、以下のよう設定になっています。

- アクセス数: 0
- 別アクセスとみなす経過時間": 0 分
- アクセス数に履かせるゲタ: 0

アクセス数 0 にリセットした場合の操作は、以下です。
...
##### パスワード無しでカウンタを作ってしまうと、もうその ID ではカウンターを作れない

例えば、`ff2` という名前のカウンタを作る場合、ブラウザの URL 欄に
...
誤って作ったカウンターや、もう必要なくなったカウンターを削除したい場合、明示的に削除することはできません。
